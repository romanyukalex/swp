...
<div class="b-inner-top-banners">
	...
	<div class="banner banner_right">
	  <div class="banner_wrap">
		<div class="message">
		  <div class="title"><?=$vendorinfo['vendor_short_description_'.$language];?><br><a href='/?page=<?=$modulename?>&action=show_vendor&vendor_id=<?=$vendorinfo['vendor_id']?>&menu=services_<?=$vendorinfo['vendor_id']?>'><?=$vendorinfo['vendor_name_'.$language];?></a></div>
		</div>
		<? 
		if($userrole!=="user"){?>
		<a class="b-button b-button_sidebar mt-40 js-window" style="position:absolute;    bottom:0;width:89%"
		<? if($userrole=="superuser") {?>href="/?page=order&action=show_product_variants&productid=<?=$product_id?>" onclick="showblock('variant_form_1');	$('#variant_form_div').arcticmodal(); return false;"<? }
...
<h2><? if ($productinfo['product_id']){
		...
		if ($vendorinfo['vendor_id']){
			...
			?><a href='/?page=$modulename&action=show_vendor&vendor_id=<?=$vendorinfo['vendor_id']?>&menu=services_<?=$vendorinfo['vendor_id']?>'><?=$vendorinfo['vendor_name_'.$language];?></a><?
		}
