<? # Скрипт парсит новую страницу torrent и сохраняет информацию в БД 
...
$tor_q=mysql_query("SELECT `id`,`date`,`status` FROM `$tableprefix-torrents` WHERE `topic_id`='".$torr_info[1]."' LIMIT 0,1;");
...
if($dnld_torr==1){ // Парсим страницу торрента
	...
	if($erase_flag==1){// Стираем строку БД, тк надо обновить торрент
		mysql_query("DELETE FROM `$tableprefix-torrents` WHERE `topic_id` = '".$torr_info[1]."';");
	...
	if($rss_config['need_apply']=="auto_confirm") {
		...
		$book_url="https://".$sitedomainname."/?page=book&topic_id=".$torr_info[1].'&from=sbscrptns';
		#Короткая ссылка на книгу
		//$book_url=file_get_contents("https://clck.ru/--?url=https://".$sitedomainname."/?page=book&topic_id=".$torr_info[1].'&from=sbscrptns');
	...
	elseif($rss_config['need_apply']=='manual_confirm') {
		...
		Для подтверждения пройдите по ссылке: <a href='https://".$sitedomainname."/?page=book&topic_id=".$torr_info[1]."'>".$torr_info['name']."</a>";
	  ...
	  ('Модерация книги ".$torr_info['name']."', 'new', 'need_moderate', 'https://".$sitedomainname."/?page=book&topic_id=".$torr_info[1]."');");
	}
	...
	if(strstr($desc_html,"ксмо")){
		...
		mysql_query("INSERT INTO `$tableprefix-portal-events` 
	  (`text`, `status`, `type`, `link`) VALUES 
	  ('Добавлена книга Эксмо ".$torr_info['name']."', 'new', 'need_moderate', 'https://".$sitedomainname."/?page=book&topic_id=".$torr_info[1]."');");
	}
	
	#Добавляем торрент
	mysql_query("INSERT INTO `$tableprefix-torrents` (`name`,`hash`, `date`, `size`, `topic_id`, `cat_id`, `cat_name`, `year`, `orig_img`, `orig_desc`,`status`) 
	...
	if($rss_config['need_apply']=="auto_confirm" and mysql_affected_rows()==1) {
		#Раз уж точно располагаем книжку на сайте и записали в БД, то давайте оповестим юзеров из подписки об этом
