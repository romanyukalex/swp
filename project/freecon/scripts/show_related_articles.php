	...
	if($pagequery['tags']) { #Есть теги к странице
		...
		if(count($page_tags_arr)>1) {#Несколько тегов у страницы
			...
			foreach($page_tags_arr as $tag){
				if ($tag) {
					$tag_q.=" or `tags` LIKE '%$tag%' or `pagetitle_ru` LIKE '%$tag%'"; //убедившись, что тег не пустой
					$has_tag=1; //флаг, что есть теги в итоге всех проверок
				}
			}
			$tag_q="and (".mb_substr($tag_q,4).")";
		} elseif(count($page_tags_arr)==1 and $page_tags_arr[0]){#Всего 1 тег и он не пустой
		...
		#Ищем статьи с тамими же словами
