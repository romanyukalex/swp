...
if ($('parallax')) {
	...
	//DD_belatedPNG.fix('#logomark, .layer, .layer div' ); // argument is a CSS 



	//Check for quickportfolio boxes and setup slideshow if exists
	
	if ($('quickportfolio')){
	
  		var myShow = new Slideshow.KenBurns('portfolio_modbox', data, {zoom:20, random:true, duration:2500, loader:false });
...
var getNavToken = function (element) {
	//get the id of the element and strip out the manifesto generated prefix
	var station = element.id.replace("navrow_pages_", "");
	station = station.replace("navrow_", "");
	return station;	
...
var init = function(method) {
	...
	modclass = document.body.className.match(/mod_([-_a-z]+)/)[1];
	subclass = document.body.className.match(/sub_([-_a-z]+)/);
	if (subclass) { 
		section = subclass[1]; 
	} else if (modclass) {
		section = modclass;
...
*/
	...
	initialize: function(el, data, options){	
		...
		if (!this.slideshow) 
			return;
		this.slideshow.setStyles({'display': 'block', 'position': 'relative', 'z-index': 0});
