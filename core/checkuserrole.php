...
if (isset($_REQUEST['login'])){// пришли данные для логина
	...
	if ($login or $password) {#Что то пришло. Проверка полученного:
		...
		if($err!==0){
			...
			$showmessage='';
			if(set('login_method')=='Скрипт'){# Особая аутентификация проекта
			...
			} elseif(set('login_method')=='LDAP'){# Доделать
			} else{ #set('login_method')=='Локальная';
				...
				$userquery_q=mysql_query("SELECT * FROM `$tableprefix-users` WHERE $loginphrase and `password`='$password' -- ".$rand.";");
				...
				elseif(mysql_num_rows($userquery_q) == 1){# Однозначно аутентифицировали
					...
					if($userquery['status']=='active'){
						...
						//else
						if($fullname){$showmessage.=$fullname;}
					...
					if($userquery2['userid']){# Пользователь с таким именем существует, значит неверен только пароль
						...
						if(set('passinptrymaxcount')<=$userquery2['passw_inp_count']){# Преышен максимальный порог количества попыток ввести пароль. Блокируем пользователя на запросы пароля
							$_SESSION['blocked_login']=time();
							$log->LogDebug('Block login (max count of trying to login)');
						}
					}
					// Что показываем пользователю?
					if(set('showpasserror')=='Пароль'){//Выясняем подробнее
						if($userquery2['userid']){ # Пользователь с таким именем существует, значит неверен только пароль
	...
	$log->LogDebug('User already logged in - userrole='.$userrole.' userid='.$userid.' login='.$login);
	...
	if(!$_SESSION['user_data']){
		$userquery=mysql_fetch_assoc (mysql_query("SELECT * FROM `$tableprefix-users` WHERE `userid`='$userid' -- ".$rand.";"));
		$_SESSION['user_data']=$userquery; // Все данные о юзере в одном сессионном массиве
		
		//Все данные юзера вытаскиваются так - $_SESSION['user_data']['gender']
