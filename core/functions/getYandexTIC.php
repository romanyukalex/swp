<?php
 /****************************************************************
  * Snippet Name : getYandexTIC		           					 * 
  * Scripted By  : RomanyukAlex		           					 * 
  * Website      : http://popwebstudio.ru	   					 * 
  * Email        : admin@popwebstudio.ru     					 * 
  * License      : GPL (General Public License)					 * 
  * Purpose 	 : Получаем Яндекс.ТИЦ любого домена			 *
  * Access		 : $domain = "http://".$_SERVER['HTTP_HOST']."/";*
  *           $tcy = getYandexTIC($domain);echo "ТИЦ: ".$tcy;	 *
  ***************************************************************/
function get_ydownload($url){
$ret = false;
...
function getYandexTIC($url){
	...
	if ($content = get_ydownload('http://bar-navig.yandex.ru/u?ver=2&url='. urlencode($url).'&show=1&post=0')){
	//echo "<pre>".'http://bar-navig.yandex.ru/u?ver=2&url='. urlencode($url).'&show=1&post=0'."</pre>";
