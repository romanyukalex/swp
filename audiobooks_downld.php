...
<div class="row flex-items-md-center" style="margin-top:20px">
	...
	<? 
	if($bookauthor) {
	$same_author_q=mysql_query("SELECT * FROM `$tableprefix-torrents` WHERE `name` like '%$bookauthor%' LIMIT 0,10;");
		if(mysql_num_rows($same_author_q)>1){
			...
			while ($same_author_books=mysql_fetch_array($same_author_q)){
				...
				?><a class="justlink" style="font:14px" href="/?page=book&topic_id=<?=$same_author_books['topic_id']?>"><?=$sa_book_info['title'];?>[<?=$same_author_books['year'];?>]</a><br><br><?
			}
		}
	}
	$same_razdel_book_q=mysql_query("SELECT * FROM `$tableprefix-torrents` WHERE `cat_id`='".$book_info['cat_id']."' ORDER BY RAND() limit 0,5;");
