 ...
 */
        ...
        $response_asid=$response[0]->id;
        $response = $this->api('wall.post',
            [
                'owner_id' => -$publicID,
                'from_group' => 1,
                'message' => "$text",
                'attachments' => "$response_asid", // uploaded image is passed as attachment
...
public function postImgToGroup($publicID, $fullServerPathToImage)
    {
        ...
        $uploadURL = $response->upload_url;
        //https://pu.vk.com/c837325/upload.php?act=do_add&mid=10153418&aid=-14&gid=0&hash=c5d7d3b6020bbe7e2f80120e8e44556c&rhash=fbee547ea8f40e5508cd9e6a569e8b8a&swfupload=1&api=1&wallphoto=1
        $output = [];
        echo  $uploadURL;
       // exec("curl -X POST -F 'photo=@$fullServerPathToImage' '$uploadURL'", $output);
        $postparam=array("file1"=>"@".$fullServerPathToImage);
