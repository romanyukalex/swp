<?php
  ...
  * Access		 : $lang=insert_module("Language_Auto_Detect","word");	*
...
if ($nitka=="1"){
	...
	$tmp_str = $param[1];// это наша строка
	if (!empty($tmp_str)){

		include_once dirname(__FILE__).'/Lang_Auto_Detect.php'; // Подключили класс
		...
		if(strstr($tmp_str,"@")){//Передали несколько слов через символ@
			$words=explode("@",$tmp_str);
			foreach($words as $word){
				$res[$word] = $test_obj->lang_detect($word);// Определили язык каждого слова
			}
			return  $res;
		} else {//Передали одно слово
			$res = $test_obj->lang_detect($tmp_str);
			if (($res == false) || ($res == null))	
			{
				return false; // Язык НЕ определен

			} else{$result[$tmp_str]=$res;
				return  $result;
			}
		}
	} else {#Нет слова для определения
		return false;
	}
 }
