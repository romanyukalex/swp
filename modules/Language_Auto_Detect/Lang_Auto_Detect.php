...
class Lang_Auto_Detect
{
	...
	private function _prepare_str($tmp_str = null)
	{
		...
		{
			//обрезать длину текста, для производительности
	...
	private function _detect_from_tables($tmp_str = null)
	{
		...
		{//echo $lang_code." "; var_dump($lang_name);
		...
		if ($this->return_all_results == true)
		{
		...
		else
			{
				...
				if($this->use_str_len_per_lang ===true){//Не важно, превышен ли порог detect_range
					//var_dump ($key);
					return $key;
				} elseif ($lang_res[$key] >= $this->detect_range){
		...
		else
			{
				...
				if ($this->use_rules === true)
				{
					...
					if ($this->use_rules_priory === true)
					{
						...
						return $res_rules; //array($res_rules, $this->lang[$res_rules]);
					}
					else
						{
							return $res;//array($res, $this->lang[$res]);
						}
				}
				else
					return $res;//array($res, $this->lang[$res]);
