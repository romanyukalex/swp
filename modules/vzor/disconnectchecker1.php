<?php
@require($_SERVER["DOCUMENT_ROOT"]."/vzor/whatbrowser.php");
if (!$block)
	{
	@require($_SERVER["DOCUMENT_ROOT"]."/core/db/dbconn.php");
	//$prov1=mysql_query("select `action`,`date`,`ip`,`userid` from `ovzor-log` where `date` in (SELECT max(`date`) FROM `ovzor-log` where unix_timestamp(`date`) BETWEEN (unix_timestamp(now()) - 120) AND (unix_timestamp(now())-60) group by `ip` order by `date` DESC) order by `date` DESC "); //грузит мускул
	//$prov2=mysql_query("select `action`,`date`,`ip`,`userid` from `ovzor-log` where `date` in (SELECT max(`date`) FROM `ovzor-log` where unix_timestamp(`date`) BETWEEN (unix_timestamp(now()) - 59) AND unix_timestamp(now()) group by `ip` order by `date` DESC) order by `date` DESC ");
...
//			else echo "Не надо проверять действие ".$lastactions1[action]." ip ".$provdata1[ip]." дата ".$provdata1[data];
		...
		if ($n==1)
				...
				//@require("/home/tribalm/public_html/vzor/payit.php");
				}
