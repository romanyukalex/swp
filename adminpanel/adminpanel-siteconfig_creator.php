...
?>
<div id="add_new_param_ap"></div>
<form id="add_new_param_form">
<table class="settings_table">

<tr class="gradient_to_top_green">
<th width="25%">Свойство</th>
<th width="75%">Значение</th>

</tr>

<tr><td class="barrel-rounded">
<label>Описание параметра</label></td>
<td class="heavy-rounded"><input type="text" name="new_var_describe" size="<?=$formsize_standart?>"></td>

</tr>


<tr><td class="barrel-rounded"><label>Тип параметра</label></td>

<td class="heavy-rounded"><select name="new_vartype"><option value="1">Input</option><option value="2">Select</option><option value="3">Checkbox</option><option value="4">Color</option></select></td></tr>

<tr><td class="barrel-rounded"><label>Значение параметра*</label></td><td class="heavy-rounded"><input type="text" name="new_var_value" title="Если тип input, пишем что угодно. Если color, то пишем цвет в формате '#f7f2e6'" size="<?=$formsize_standart?>"></td></tr>

<tr><td class="barrel-rounded"><label>Варианты значений параметра*</label></td><td class="heavy-rounded"><input type="text" name="new_var_value_variants" title="Указывается, если тип select, перечисляем значения через ';;'" size="<?=$formsize_standart?>"></td></tr>

<? /*
<tr><td class="barrel-rounded"><label>Максим количество символов</label></td><td class="heavy-rounded"><input type="text" name="new_var_formmaxlegth" title="Указывается для input. Для select будет считать количество, исходя из вариантов выборки" size="<?=$formsize_standart?>"></td></tr>
*/?>

<tr><td class="barrel-rounded"><label>Как будет называться переменная в API*</label></td><td class="heavy-rounded"><input type="text" name="new_var_system-param-name" size="<?=$formsize_standart?>"></td></tr>

<tr><td>
<!--<label>Описание переменной в API*</label></td><td class="heavy-rounded"><input type="text" name="new_var_system-param-describe"></td></tr><tr><td>-->
<label>Относится к</label></td><td class="heavy-rounded"><select name="new_var_depend"><option value="design">Design</option><option value="user">User parameters</option><option value="system">System</option><option value="SEO">SEO</option><option value="other" selected>Other</option></select></td></tr>

<tr><td class="barrel-rounded"><label>Показывать админу сайта</label></td><td class="heavy-rounded"><select name="new_var_ShowToSiteAdmin"><option value="1">Показывать</option><option value="0">Не показывать</option></select></td></tr>

<tr><td class="barrel-rounded"><label>Может быть пустым?</label></td><td class="heavy-rounded"><select name="new_var_maybeempty"><option value="y">Может</option><option value="n">Не может</option></select></td></tr>

<tr><td class="barrel-rounded">
<!--<label>Размер size для формы в админке</label></td><td class="heavy-rounded"><input type="text" name="new_var_formsize">-->
<label>Пример</label></td><td class="heavy-rounded"><input type="text" name="new_var_example" title="Если не указать явно, то будет равно 'Значению параметра'" size="<?=$formsize_standart?>"></td></tr>

<tr><td class="barrel-rounded">
<label>Модуль</label></td><td class="heavy-rounded">
<select name="new_var_module_id"><option value="-" checked>-</option>
<? 
mysql_data_seek($avmodulesqry,0);
while($avmodulesresult=mysql_fetch_array($avmodulesqry)){
	?><option value="<?=$avmodulesresult['module_id']?>"><?=$avmodulesresult['module_description']?> [<?=$avmodulesresult['modulename']?>]</option><?
}?>
</select>
</td></tr>
...
<label>Компания</label></td><td class="heavy-rounded">

</td></tr>

<tr><td></td><td><a href="/" class="large button orange light-rounded" onclick="saveform3('','add_new_param_form','add_new_param_ap','add_new_param_ap','adminpanel','add_new_param','no_resetform','hideform'); return false;">Создать новый параметр системы</a></td></tr>
